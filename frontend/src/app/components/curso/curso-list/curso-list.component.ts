...
@Component({
  selector: 'app-curso-list', // O nome da tag HTML para este componente (ex: <app-curso-list></app-curso-list>).
  ...
  templateUrl: './curso-list.component.html', // Aponta para o arquivo HTML que define a aparência do componente.
  styleUrls: ['./curso-list.component.css']   // Aponta para o arquivo CSS com os estilos específicos deste componente.
})
// A classe implementa 'OnInit', o que significa que ela deve ter um método chamado 'ngOnInit'.
export class CursoListComponent implements OnInit {
  
  // Declara uma propriedade chamada 'cursos'. Será um array de objetos do tipo 'Curso', inicializado como um array vazio.
  cursos: Curso[] = [];
   ...
   * @param cursoService Uma instância do nosso serviço para buscar dados do backend.
   ...
   */
  ngOnInit(): void {
    this.loadCursos(); // Chama o método para carregar os cursos do backend.
  }

  /**
   * Método responsável por buscar a lista de cursos do backend.
   */
  loadCursos(): void {
    // Usa o serviço 'cursoService' para chamar o método 'getCursos()'.
    // O método '.subscribe()' "escuta" a resposta da chamada à API.
    this.cursoService.getCursos().subscribe(data => {
      // Quando os dados chegam do backend ('data'), eles são atribuídos à nossa propriedade 'cursos'.
  ...
  /**
   * Método chamado quando o botão "Novo Curso" é clicado.
   */
  novoCurso(): void {
   ...
   * @param id O ID do curso a ser editado, que é opcional.
   */
   ...
   * @param id O ID do curso a ser excluído, que é opcional.
   */
  excluirCurso(id?: string): void {
    // Verifica se um ID foi passado e abre uma janela de confirmação para o usuário.
    if (id && confirm('Tem certeza que deseja excluir este curso?')) {
      ...
      this.cursoService.deleteCurso(id).subscribe(() => {
        ...
        // Chamamos 'loadCursos()' novamente para recarregar a lista da tabela,
